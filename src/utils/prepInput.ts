...
export function getInputModelsOrError(
  ...
  if (reqBody.left && reqBody.right) {
    console.log("NEW SESSION - session regenerate");
...
export function prepForPatch(
  reqBody: unknown,
  reqSession: Session & Partial<SessionData>
): {leftIds: unknown[], rightIds: unknown[], diffmodel: DiffModel} | string {
  const sessionDiffModel = reqSession.diffModel;
  const { leftIds, rightIds } = reqBody as { leftIds: unknown, rightIds: unknown };

  if (!Array.isArray(leftIds) || !Array.isArray(rightIds)) {
    return 'invalid input';
  } else if (sessionDiffModel === undefined) {
    return 'invalid input';
  } else {
    return({leftIds: leftIds, rightIds: rightIds, diffmodel: sessionDiffModel});
  }
}
...
function gatherUpMoveOpsFromDeleteAdd(
  ...
  for (const diffAdd of flattenAdd) {
    const foundDelete = flattenDelete.find(
      (diffDelete) =>
        (diffDelete.opInfo.value as Record<string, undefined>)[CONFIG.IDENTIFIER]
